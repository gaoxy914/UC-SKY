...
int main(int argc, char const *argv[]) {
    ...
    if (strcmp(argv[1], "gen-data") == 0) {
        ...
        int m = atoi(argv[2]);
        for (double alpha = 0.2; alpha <= 0.8; alpha += 0.15) {
            UCSKY_Solver solver1(2, 2000000, alpha, 0);
            solver1.gen_ind_data_2d(m);
            solver1.write_data("data/ind/");
            /* UCSKY_Solver solver2(2, 2000000, alpha, 0);
            solver2.gen_anti_data_2d(m);
            solver2.write_data("data/anti/");
            UCSKY_Solver solver3(2, 2000000, alpha, 0);
            solver3.gen_corr_data_2d(m);
            solver2.write_data("data/corr/"); */
        }
    } else {
